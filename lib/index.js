...
function getJWKSSecret (jwksUrl) {
  return function (request, decodedToken, jwtHeader, callback) {
    get.concat({
      ...
      callback(null, '-----BEGIN PUBLIC KEY-----\n'+match.x5c[0]+'\n-----END PUBLIC KEY-----\n');
    })
  }
}

function authorize(options) {
  options = { decodedPropertyName: 'decoded_token', ...options};
...
function getSecret(request, secret, token, callback) {
  if (typeof secret === 'function') {
    ...
    if (secret.length === 3) {
      return secret(request, payload, callback);
    } else if (secret.length === 4) {
      return secret(request, payload, header, callback);
    } else {
      return callback({ code: 'bad_secret_function', message: `Secret function has ${secret.length} arguments instead of 3 or 4` });
    }
  } else {
