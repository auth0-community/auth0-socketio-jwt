...
var InvalidTokenError = require('./InvalidTokenError.js');
...
function noQsMethod(options) {
  ...
  return function (socket) {
    ...
    socket.on('authenticate', function (data) {
      ...
      onJwtVerificationReady = function (err, decoded) {

        if (err) {
          return onError(err, 'invalid_token');
        }

        if (!decoded.client_id && !decoded.salesforce_id) {
          return onError({ message: 'Invalid Token' }, 'invalid_token');
        }
...
function authorize(options, onConnection) {
  ...
  return function (data, accept) {
    ...
    var onJwtVerificationReady = function (err, decoded) {
      ...
      if (!decoded.salesforce_id && !decoded.client_id) {
        error = new InvalidTokenError('invalid_token', {
